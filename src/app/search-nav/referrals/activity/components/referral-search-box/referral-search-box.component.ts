...
import { DateRangeValidators } from '../../../../../claims/abm/referral/make-a-referral/components/date-range-form/dateRangeValidators';
import {
...
export class ReferralSearchBoxComponent
  ...
  implements OnInit, AfterViewInit
{
  ...
  _dateRangeChange(range: { fromDate: Date; toDate: Date }) {
    if (range && range.fromDate && range.toDate) {
      if (
        moment(new Date(range.fromDate)).startOf('day').toString() !==
      ...
      if (
        moment(new Date(range.toDate)).startOf('day').toString() !==
